...
function salvaSpazio(casellaCliccata, turno) {
    let move = {}
    if (turno == 'bianco') {
...
function mossePedoneBianco(casellaCliccata, mosseLegal) {
    ...
    if (gameData[casellaCliccata].casella == false) {
        mosse.push(salvaSpazio(casellaCliccata, 'bianco'))
...
function mossePedoneNero(casellaCliccata, mosseLegal) {
    ...
    if (casellaCliccata > 63 || casellaCliccata < 0) return mosse
    if (gameData[casellaCliccata].casella == false) {
        mosse.push(salvaSpazio(casellaCliccata, 'nero'))
...
// console.log(mossePossibili(turno))

function mossePossibili(turno) {
    let mosseSottoAttacco = []
    let mosse
    for(let i = 0; i < gameData.length; i++) {
        if (gameData[i].colorePedone == turno) {
            if (gameData[i].tipoPedone == 'RE' || gameData[i].tipoPedone == 'REN') {
                mosse = mosseRe(i, 'si', gameData)
                for(let j = 0; j < mosse.length; j++) {
                    mosse[j].casellaCliccataMossa = i
                    mosse[j].tipoPedone = gameData[i].tipoPedone
                }
                mosseSottoAttacco = mosseSottoAttacco.concat(mosse)
                break
            }
        }
    }
    ...
    for(let i = 0; i < gameData.length; i++) {
        if (gameData[i].colorePedone == turno) {
            switch (gameData[i].tipoPedone) {
                case 'PB':
                    mosse = mossePedoneBianco(i, 'si', gameData)
                    for(let j = 0; j < mosse.length; j++) {
                        mosse[j].casellaCliccataMossa = i
                        mosse[j].tipoPedone = 'PB'
                    }
                    mosseSottoAttacco = mosseSottoAttacco.concat(mosse)
                    break;
                case 'PN':
                    mosse = mossePedoneNero(i, 'si', gameData)
                    for(let j = 0; j < mosse.length; j++) {
                        mosse[j].casellaCliccataMossa = i
                        mosse[j].tipoPedone = 'PN'
                    }
                    mosseSottoAttacco = mosseSottoAttacco.concat(mosse)
                    break;
                case 'RE':
                    // mosseSottoAttacco = mosseSottoAttacco.concat(mosseRe(i, undefined, 'si'))
                    break
                case 'T':
                    mosse = mosseTorre(i, 'si', gameData)
                    for(let j = 0; j < mosse.length; j++) {
                        mosse[j].casellaCliccataMossa = i
                        mosse[j].tipoPedone = 'T'
                    }
                    mosseSottoAttacco = mosseSottoAttacco.concat(mosse)
                    break
                case 'R':
                    mosse = mosseRegina(i, 'si', gameData)
                    for(let j = 0; j < mosse.length; j++) {
                        mosse[j].casellaCliccataMossa = i
                        mosse[j].tipoPedone = 'R'
                    }
                    mosseSottoAttacco = mosseSottoAttacco.concat(mosse)
                    break
                case 'A':
                    mosse = mosseAlfiere(i, 'si', gameData)
                    for(let j = 0; j < mosse.length; j++) {
                        mosse[j].casellaCliccataMossa = i
                        mosse[j].tipoPedone = 'A'
                    }
                    mosseSottoAttacco = mosseSottoAttacco.concat(mosse)
                    break
                case 'C':
                    mosse = mosseCavallo(i, 'si', gameData)
                    for(let j = 0; j < mosse.length; j++) {
                        mosse[j].casellaCliccataMossa = i
                        mosse[j].tipoPedone = 'C'
                    }
                    mosseSottoAttacco = mosseSottoAttacco.concat(mosse)
                    break
                case 'REN':
...
// console.log(miniMax(gameData, true, 2))
function brrr() {
    console.log(miniMax(gameData, 1, true, 'nero'))
}

function miniMax(gameData, depth, isMaximizer) {
    if (depth === 0) return { evaluation : Math.random() }
    //if( isWinner(turno, player.man)      ) return {evaluation : -10}
    //if( checkTie()                          ) return {evaluation : 0}
    iter ++ 
    ...
    let SPACES_MOVES = mossePossibili(turno)
    let moves = []
    depth --

    for (let i = 0; i < SPACES_MOVES.length; i++) {
        var backupCasellaCliccata = gameData[SPACES_MOVES[i].casellaCliccataMossa]
        var backupCasellaSpostamento = gameData[SPACES_MOVES[i].id]

        gameData[SPACES_MOVES[i].id].casella = true
        gameData[SPACES_MOVES[i].id].tipoPedone = SPACES_MOVES[i].tipoPedone
        gameData[SPACES_MOVES[i].id].colorePedone = gameData[SPACES_MOVES[i].casellaCliccataMossa].colorePedone
        
        gameData[SPACES_MOVES[i].casellaCliccataMossa].casella = false
        gameData[SPACES_MOVES[i].casellaCliccataMossa].tipoPedone = ''
        gameData[SPACES_MOVES[i].casellaCliccataMossa].colorePedone = ''
        ...
        if (isMaximizer){
            move.evaluation = miniMax(gameData, depth, false, 'bianco').evaluation
        } else {
            move.evaluation = miniMax(gameData, depth, true, 'nero').evaluation
        }

        gameData[SPACES_MOVES[i].casellaCliccataMossa] = backupCasellaCliccata
        gameData[SPACES_MOVES[i].id] = backupCasellaSpostamento
...
function mossaFatta(SPACES_MOVES, gameData, i, turno) {
    gameData[SPACES_MOVES[i].casellaCliccataMossa].casella = false
    gameData[SPACES_MOVES[i].casellaCliccataMossa].tipoPedone = ''
    gameData[SPACES_MOVES[i].casellaCliccataMossa].colorePedone = ''
    ...
    gameData[SPACES_MOVES[i].id].casella = true
    gameData[SPACES_MOVES[i].id].tipoPedone = SPACES_MOVES[i].tipoPedone
    gameData[SPACES_MOVES[i].id].colorePedone = turno
